...
import { autoDraft, checkDraftForRestore } from "./auto-draft.js";
import { readDraft } from "../crud/drafts.js";
...
describe("autoDraft", () => {
  ...
  it("返回一个 AgentPlugin（有 onAfterSend）", () => {
    const plugin = autoDraft({ draftsDir: dir, agentId: "agent-1", modelId: "m1" });
  ...
  it("写盘失败时不抛异常（目录不存在时自动创建）", async () => {
    ...
    const plugin = autoDraft({ draftsDir: nestedDir, agentId: "agent-1", modelId: "m1" });
...
describe("checkDraftForRestore", () => {
  ...
  it("过期后自动清理并返回 null", async () => {
    ...
    const { writeDraft } = await import("../crud/drafts.js");
