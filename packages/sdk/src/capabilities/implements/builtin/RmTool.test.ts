...
describe("RmTool", () => {
  it("工具名称和描述正确", () => {
    const tool = new RmTool(makeEnv());
    expect(tool.function.name).toBe("rm");
    expect(tool.function.description).toBe("删除文件或目录");
  });
  ...
  it("递归删除目录", async () => {
    ...
    const result = await tool.call({ path: dir, recursive: true });
    expect(result).toBe("success");
  ...
  it("不递归时删除非空目录返回错误", async () => {
    ...
    try { await tool.call({ path: dir, recursive: true }); } catch {}
  });
