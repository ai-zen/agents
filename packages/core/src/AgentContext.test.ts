...
describe("AgentContext", () => {
  ...
  it("缺少 client 时应抛出错误", () => {
    expect(() => {
      new AgentContext({ client: undefined as any, model: "gpt-4" });
    }).toThrow("AgentContext must have a client");
  });

  it("缺少 model 时应抛出错误", () => {
    expect(() => {
      new AgentContext({ client: {} as any, model: undefined as any });
  ...
  it("应接受自定义配置", () => {
    ...
    expect(ctx.modelConfig).toEqual({ temperature: 0.7 });
  ...
  describe("append", () => {
    it("应添加消息并返回最后一条", () => {
      const ctx = new AgentContext({ client: {} as any, model: "gpt-4" });
