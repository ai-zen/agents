 ...
 * - parsedArgs / parseError：由 tool_call.function.arguments JSON 解析而来
...
export class ToolCallContext {
   ...
   * 解析失败且 allowJsonParseError=true 时为 undefined（此时 parseError 有值）；
  ...
  /**
   * 参数 JSON 解析错误信息。
   * 仅当 allowJsonParseError=true 且解析失败时存在；此时 parsedArgs 为 undefined，
  ...
  constructor(options: {
    ...
    if (this.function_call.arguments) {
      try {
        this.parsedArgs = JSON.parse(this.function_call.arguments);
      } catch (e: any) {
        if (options.allowJsonParseError) {
          this.parsedArgs = undefined;
